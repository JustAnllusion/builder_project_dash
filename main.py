...
from utils import compute_avg_depletion_curve, compute_smart_group_name, convert_df_to_csv, convert_df_to_excel, convert_df_to_parquet, download_ui, get_top_categories, hex_to_rgba, load_data, load_depletion_curves, numeric_filter_widget, safe_multiselect
from charts import build_histogram, build_scatter, build_depletion_chart

st.set_page_config(page_title="Анализ данных Москвы", layout="wide", initial_sidebar_state="expanded")
st.markdown("""
<style>
...
st.sidebar.markdown("<div class='sidebar-header'>Фильтрация</div>", unsafe_allow_html=True)
with st.sidebar.expander("Глобальная фильтрация", expanded=True):
    ...
    selected_filter_columns = st.multiselect("Признаки для фильтрации", options=list(house_data.columns), default=[])
    global_filters = {}
    for col in selected_filter_columns:
        if pd.api.types.is_numeric_dtype(house_data[col]):
            col_data = house_data[col].dropna()
            global_filters[col] = numeric_filter_widget(col, col_data, key_prefix="global_filter")
        else:
            ...
            selected_options = st.multiselect(f"Выберите значения для «{col}»", options=all_categories, default=[], key=f"global_filter_{col}")
...
for col, filter_val in active_global_filters.items():
    if isinstance(filter_val, tuple):
        global_filtered_data = global_filtered_data[(global_filtered_data[col] >= filter_val[0]) & (global_filtered_data[col] <= filter_val[1])]
    else:
        if filter_val:
            global_filtered_data = global_filtered_data[global_filtered_data[col].isin(filter_val)]
    ...
    if json_file is not None and json_file.name not in st.session_state.processed_json_files:
        try:
            ...
            for group_name, ids in groups_from_json.items():
                group_color = st.color_picker(f"Цвет для группы «{group_name}»", value="#0000FF", key=f"uploaded_group_color_{group_name}")
                group_df = house_data[house_data["house_id"].isin(ids)].copy()
                new_group = {"group_name": group_name, "group_color": group_color, "selected_filter_columns": [], "column_filters": {}, "filtered_data": group_df.copy(), "is_static": True, "base_data": group_df.copy()}
...
if st.session_state.dynamic_groups:
    ...
    for idx, group in enumerate(st.session_state.dynamic_groups):
        with st.sidebar.expander(f"{group['group_name']}", expanded=False):
            ...
            new_name = st.text_input("Название группы", value=group["group_name"], placeholder=smart_placeholder, key=f"group_name_{idx}")
            if new_name.strip() == "":
            ...
            group["group_color"] = st.color_picker("Цвет группы", value=group["group_color"], key=f"group_color_{idx}")
            group["selected_filter_columns"] = st.multiselect("Выберите признаки для фильтрации", options=list(house_data.columns), default=group.get("selected_filter_columns", []), key=f"group_filter_columns_{idx}")
            column_filters = {}
            for col in group["selected_filter_columns"]:
                if pd.api.types.is_numeric_dtype(house_data[col]):
                    ...
                    popular_categories = get_top_categories(group["base_data"], col, top_n=5)
                    st.write(f"Популярные варианты для «{col}»: {', '.join(popular_categories)}")
                    selected_options = st.multiselect(f"Выберите значения для «{col}»", options=all_categories, default=[], key=f"group_filter_{idx}_{col}")
                    column_filters[col] = selected_options
            group["column_filters"] = column_filters
            base_df = group["base_data"].copy() if group.get("is_static", False) else house_data.copy()
            ...
            group_configs[group["group_name"]] = {"column_filters": group["column_filters"], "filtered_data": group["filtered_data"], "vis": {"color": group["group_color"], "opacity": 200, "radius": 50, "show": True}}

tab_data, tab_map, tab_analysis, tab_prediction, tab_clustering = st.tabs(["Данные", "Карта объектов", "Анализ", "Предсказание", "Кластеризация"])

with tab_data:
    st.markdown("<div class='section-header'>Табличное представление данных</div>", unsafe_allow_html=True)
    ...
    num_rows = st.number_input("Количество строк для отображения", min_value=1, max_value=max_table_rows if max_table_rows > 0 else 1, value=default_rows, step=1)
    percentage = (max_table_rows / total_rows * 100) if total_rows > 0 else 0
    st.write(f"Найдено строк: **{max_table_rows}** из **{total_rows}** ({percentage:.2f}%)")
    visible_columns = ["project", "house_id", "developer", "class", "start_sales", "ndeals", "deals_sold", "mean_area", "mean_price", "mean_selling_time", "floor"]
    available_columns = [col for col in visible_columns if col in main_table.columns]
    display_df = main_table[available_columns].copy() if available_columns else main_table.copy()
    ...
    if group_configs:
        ...
        for grp, config in group_configs.items():
            ...
            with st.expander(f"{grp} (строк: {len(grp_df)})", expanded=False):
                ...
                available_columns_grp = [col for col in visible_columns if col in grp_df.columns]
                display_grp_df = grp_df[available_columns_grp].head(num_rows).copy() if available_columns_grp else grp_df.head(num_rows).copy()
...
with tab_map:
    st.markdown("<div class='section-header'>Интерактивная карта объектов</div>", unsafe_allow_html=True)
    ...
    if not required_cols.issubset(house_data.columns):
        ...
        cols[0].markdown(f"<div style='width:20px;height:20px;background:{global_vis_color};border-radius:3px;'></div>", unsafe_allow_html=True)
        ...
        for idx, grp in enumerate(group_configs.keys(), start=1):
            ...
            cols[idx].markdown(f"<div style='width:20px;height:20px;background:{group_configs[grp]['vis']['color']};border-radius:3px;'></div>", unsafe_allow_html=True)
        points_for_center = pd.DataFrame()
        global_points = global_filtered_data.dropna(subset=["latitude", "longitude", "house_id"]).copy()[["latitude", "longitude", "house_id"]]
        layers = []
        if toggle_global and not global_points.empty:
            global_layer = pdk.Layer("ScatterplotLayer", data=global_points, get_position=["longitude", "latitude"], get_radius=global_radius, get_fill_color=hex_to_rgba(global_vis_color, global_opacity), pickable=True)
            layers.append(global_layer)
            points_for_center = pd.concat([points_for_center, global_points], ignore_index=True)
        for grp, config in group_configs.items():
            if not toggles.get(grp, False):
                continue
            grp_data = config["filtered_data"].dropna(subset=["latitude", "longitude", "house_id"]).copy()[["latitude", "longitude", "house_id"]]
            if not grp_data.empty:
                key_radius = f"grp_radius_{grp}"
                key_opacity = f"grp_opacity_{grp}"
                grp_layer = pdk.Layer("ScatterplotLayer", data=grp_data, get_position=["longitude", "latitude"], get_radius=st.session_state[key_radius], get_fill_color=hex_to_rgba(config["vis"]["color"], st.session_state[key_opacity]), pickable=True)
                layers.append(grp_layer)
                points_for_center = pd.concat([points_for_center, grp_data], ignore_index=True)
        if not layers:
            ...
            tooltip = {"html": "<b>ID: {house_id}</b>", "style": {"backgroundColor": "steelblue", "color": "white", "fontSize": "14px"}}
            deck = pdk.Deck(layers=layers, initial_view_state=view_state, tooltip=tooltip, map_style="https://basemaps.cartocdn.com/gl/positron-gl-style/style.json")
        ...
        if st.session_state.show_map_settings:
            with st.container():
                ...
                new_global_radius = col1.slider("Размер точек", min_value=10, max_value=200, value=global_radius, step=5, key="global_radius_slider")
                new_global_opacity = col2.slider("Яркость", min_value=0, max_value=255, value=global_opacity, step=5, key="global_opacity_slider")
                st.session_state["global_radius"] = new_global_radius
                st.session_state["global_opacity"] = new_global_opacity
                for grp in group_configs.keys():
                    st.markdown(f"**{grp}:**")
                    col1, col2 = st.columns(2)
                    new_grp_radius = col1.slider("Размер точек", min_value=10, max_value=200, value=st.session_state[f"grp_radius_{grp}"], step=5, key=f"{grp}_radius_slider")
                    new_grp_opacity = col2.slider("Яркость", min_value=0, max_value=255, value=st.session_state[f"grp_opacity_{grp}"], step=5, key=f"{grp}_opacity_slider")
    ...
    with analysis_mode_tabs[0]:
        ...
        for idx, config in enumerate(st.session_state.graph_configs):
            with st.container():
                ...
                with colA:
                    config["name"] = st.text_input("Название графика", value=config["name"], key=f"chart_name_{idx}")
                with colB:
                    config["chart_type"] = st.selectbox("Тип графика", options=["Гистограмма", "Скатерплот", "Кривая выбытия"], index=["Гистограмма", "Скатерплот", "Кривая выбытия"].index(config["chart_type"]), key=f"chart_type_{idx}")
                ...
                config["selected_groups"] = st.multiselect("Группы для анализа", options=available_groups, default=safe_default, key=f"chart_groups_{idx}")
                with st.expander("Настройки графика", expanded=False):
                    if config["chart_type"] == "Гистограмма":
                        ...
                        config["histogram"]["column"] = st.selectbox("Признак для гистограммы", options=numeric_cols, index=(numeric_cols.index(default_col) if default_col in numeric_cols else 0), key=f"hist_column_{idx}")
                        config["histogram"]["log_transform"] = st.checkbox("Логарифмировать", value=config["histogram"].get("log_transform", False), key=f"hist_log_{idx}")
                        config["histogram"]["remove_outliers"] = st.checkbox("Удалить выбросы", value=config["histogram"].get("remove_outliers", False), key=f"hist_outliers_{idx}")
                        if config["histogram"].get("remove_outliers", False):
                            config["histogram"]["lower_q"] = st.slider("Нижний квантиль", min_value=0.0, max_value=0.3, value=config["histogram"].get("lower_q", 0.05), step=0.01, key=f"hist_lq_{idx}")
                            config["histogram"]["upper_q"] = st.slider("Верхний квантиль", min_value=0.7, max_value=1.0, value=config["histogram"].get("upper_q", 0.95), step=0.01, key=f"hist_uq_{idx}")
                        config["histogram"]["normalize"] = st.checkbox("Нормировать гистограмму", value=config["histogram"].get("normalize", False), key=f"hist_normalize_{idx}")
                        ...
                        config["scatter"]["x"] = st.selectbox("Ось X", options=numeric_cols, index=(numeric_cols.index(default_x) if default_x in numeric_cols else 0), key=f"scatter_x_{idx}")
                        config["scatter"]["y"] = st.selectbox("Ось Y", options=numeric_cols, index=(numeric_cols.index(default_y) if default_y in numeric_cols else 0), key=f"scatter_y_{idx}")
                        config["scatter"]["normalize"] = st.checkbox("Нормировать (привести к квантилям)", value=config["scatter"].get("normalize", False), key=f"scatter_normalize_{idx}")
                    elif config["chart_type"] == "Кривая выбытия":
                        st.markdown("Настройки кривой выбытия", unsafe_allow_html=True)
                        config["depletion"]["show_individual"] = st.checkbox("Показать индивидуальные кривые", value=config["depletion"].get("show_individual", False), key=f"depletion_individual_{idx}")
                if st.button(f"Удалить график {config['name']}", key=f"delete_chart_{idx}"):
                    st.session_state.graph_configs.pop(idx)
                    st.experimental_rerun()
                selected_groups = config["selected_groups"]
                color_domain = selected_groups
                color_range = ["#FF0000" if g == "Глобальный" else group_configs[g]["vis"]["color"] for g in selected_groups]
                if config["chart_type"] == "Гистограмма":
...
with tab_prediction:
    st.markdown("<div class='section-header'>Предсказание</div>", unsafe_allow_html=True)
    st.info("Раздел 'Предсказание' находится в разработке.")

with tab_clustering:
    st.markdown("<div class='section-header'>Кластеризация</div>", unsafe_allow_html=True)
