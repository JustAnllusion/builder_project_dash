...
def render_map_tab(global_filtered_data: pd.DataFrame, house_data: pd.DataFrame, group_configs: dict):
    st.markdown(
    ...
    for grp, config in group_configs.items():
        ...
        if not grp_data.empty:
            ...
            grp_layer = pdk.Layer(
                ...
                get_fill_color=hex_to_rgba(config["vis"]["color"], st.session_state[key_opacity]),
                pickable=True,
            )
